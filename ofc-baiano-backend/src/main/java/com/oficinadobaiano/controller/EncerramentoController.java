...
public class EncerramentoController {
    ...
    public ResponseEntity<Corpo> findAll() {
        ...
        List<Encerramento> encerramentos = encerramentoService.findAll();
        response.setSuccess(true);
        response.setData(encerramentos);
    ...
    public ResponseEntity<Corpo> findById(@PathVariable Long id){
        Optional<Encerramento> encerramento = encerramentoService.findById(id);
        Encerramento e = encerramento.get();
        Corpo response = new Corpo<>();
        response.setSuccess(true);
        response.setData(e);
        return ResponseEntity.status(HttpStatus.OK).body(response);
    ...
    @SuppressWarnings({ "rawtypes", "unchecked" })
    @PostMapping
    public ResponseEntity<Corpo> create(@RequestBody Encerramento encerramento){
        encerramento.setId(null);
        Encerramento e = encerramentoService.save(encerramento);
        Corpo response = new Corpo<>();
        response.setSuccess(true);
        response.setData(e);
        return ResponseEntity.status(HttpStatus.OK).body(response);
    ...
    @SuppressWarnings({ "rawtypes", "unchecked" })
    @PutMapping
    public ResponseEntity<Corpo> update(@RequestBody Encerramento encerramento){
        Encerramento e = encerramentoService.update(encerramento);
        Corpo response = new Corpo<>();
        response.setSuccess(true);
        response.setData(e);
        return ResponseEntity.status(HttpStatus.OK).body(response);
